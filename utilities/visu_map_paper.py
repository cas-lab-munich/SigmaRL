...
def visualize_and_save_map(
    ...
    # Set up the plot
    plt.figure(
        figsize=(figsize_x, figsize_x * aspect_ratio)
    ...
    if is_show_vehicles:
        ...
        for k in range(len(lanelet_ids)):
            ...
            vertices = get_rectangle_vertices(
                center=pos, yaw=yaw, width=w, length=l, is_close_shape=True
            ).squeeze(0)
    ...
    if is_save_fig:
        ...
        file_name = "cpm.png"
        plt.savefig(
            file_name, format="png", bbox_inches="tight", transparent=True, dpi=600
        )
...
# Parse the XML file
def get_lanelets_data(
    is_save_fig=False, is_visualize=False, is_show_vehicles=True, **kwargs
):
    ...
    if is_visualize | is_save_fig:
        visualize_and_save_map(
            lanelets, intersection_info, is_save_fig, is_visualize, is_show_vehicles
        )
