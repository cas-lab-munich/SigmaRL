...
class MapManager:
    def __init__(self, **kwargs):
        self._scenario_type = kwargs.pop("scenario_type", "CPM_entire")  # One of {"CPM_entire", "CPM_mixed", "intersection_1", "design you own map and name it here"}
    ...
    def determine_current_lanelet(self, agent_pos):
        ...
        max_length = max(len(lanelet['center_line']) for lanelet in self.parser.lanelets_all)

        # Pad all center_line tensors to the same length
        lanelet_centers_padded = [
            pad(lanelet['center_line'], (0, 0, 0, max_length - len(lanelet['center_line'])))
            for lanelet in self.parser.lanelets_all
        ]
        lanelet_centers_padded = torch.stack(lanelet_centers_padded)  # Shape (num_lanelets, max_length, 2)

        # Expand agent positions and lanelet centers for distance calculation
        agent_pos_expanded = agent_pos.unsqueeze(2).unsqueeze(3)  # Shape (batch_size, num_agents, 1, 1, 2)
        lanelet_centers_expanded = lanelet_centers_padded.unsqueeze(0).unsqueeze(0)  # Shape (1, 1, num_lanelets, max_length, 2)
        ...
        min_dists, _ = torch.min(dists, dim=3)  # Shape (batch_size, num_agents, num_lanelets)

        # Find the lanelet with the minimum distance for each agent
        self.current_lanelet_idx = torch.argmin(min_dists, dim=2).unsqueeze(2)  # Shape (batch_size, num_agents, 1)
    ...
    def determine_masked_agents_by_lanelets(self, agent_idx, nearing_agents_indices):
        ...
        for env_idx in range(self.current_lanelet_idx.size(0)):
            ...
            for near_idx in range(nearing_agents_indices.size(1)):
                ...
                if near_lanelet not in self.parser.neighboring_lanelets_idx[ego_lanelet_idx]:
