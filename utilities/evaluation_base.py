...
from utilities.helper_training import SaveData, find_the_highest_reward_among_all_models, get_model_name
...
class Evaluation:
    ...
    def __init__(self, **kwargs):
        ...
        self.where_to_save_eva_results = kwargs.pop("where_to_save_eva_results", "outputs")
        ...
        self.idx_our = next((i for i, path in enumerate(self.model_paths) if "our" in path), -1) # Index of our model
        ...
        idx_our_model = next((i for i, s in enumerate(self.model_paths) if "our" in s), None)
    ...
    def _get_simulation_outputs(self):
        ...
        path_eval_out_td = self.parameters.where_to_save + self.parameters.model_name + f"_out_td_{self.parameters.scenario_type}.pth"
        ...
        if os.path.exists(path_eval_out_td) & (not self.is_render):
            ...
            if self.parameters.is_save_simulation_video:
                ...
                save_video(f"{self.model_i_path}{self.video_names[self.model_i]}_{self.scenario_type}", frame_list, fps=1 / self.parameters.dt)
    ...
    def plot(self):
        ...
        for i in range(data_np.shape[0]):
            ...
            plt.plot(smoothed_reward, label=self.legends[self.models_selected[i]], color=colors[i], linestyle="-", linewidth=0.8)

        plt.xlim([0, data_np.shape[1]])
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        
        plt.legend(bbox_to_anchor=(0.5, 1.88), loc='upper center', fontsize=9, ncol=1, handleheight=0.7, handletextpad=0.5, labelspacing=0.3)
        ...
        if (num_models_selected == 7) or (num_models_selected == 4):
            ax.legend([parts1["bodies"][0], parts2["bodies"][0], parts3["bodies"][0]], ['Total', 'Agent-agent', 'Agent-boundary'], loc='upper center', bbox_to_anchor=(0.5, 1.2), fontsize=8, ncol=3)
        else:
            ax.legend([parts1["bodies"][0], parts2["bodies"][0], parts3["bodies"][0]], ['Total', 'Agent-agent', 'Agent-boundary'], loc='upper right', fontsize=8)
        ...
        print(colored(f"Logging had been saved under ", "grey"), colored(self.where_to_save_logging, "blue"))
