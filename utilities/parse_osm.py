...
class ParseOSM(MapParseBase):
    ...
    """
    def __init__(self, map_path, device, scale, width, reference_paths_ids):
    ...
    def _process_map_data(self):
        map_data_tmp = []
        """Calculate the relevant data for each center line and store them in self.map_data."""
        ...
        self.map_data = [list(d.values())[0] for d in sorted_list]
    ...
    def _get_reference_paths(self):
        ...
        for ref_path in self._reference_paths_ids:
            ...
            # if (len(ref_path) > 1) and (ref_path[0] == ref_path[-1]):
            #     is_loop = True
            # else:
            #     is_loop = False
            ...
            #     for way_key, way in self._ways.items():
            #         if way['lanes'] == way_id:  # Iterate over to find the target
            #             for node_idx in range(len(way['nodes'])):
            #                 node_id = way['nodes'][node_idx]
                            
            #                 if path_idx == 0:
            #                     center_line_points.append(self._nodes[node_id])
            #                 elif path_idx >= 1:
            #                     if (node_idx != 0):  # Avoid repeated nodes, since the first node of one lane overlaps with the last node of its predecessor lane
            #                         center_line_points.append(self._nodes[node_id])
            #             break
            ...
            #         'reference_lanelets': torch.tensor(reference_lanelets, device=self._device, dtype=torch.int32),
            #         'center_line': center_line,
            #         'center_line_yaw': center_line_yaw,
            #         'center_line_vec_normalized': center_line_vec_normalized,
            #         'center_line_vec_mean_length': center_line_vec_mean_length,
            #         'left_boundary': left_boundary,
            #         'right_boundary': right_boundary,
            #         'left_boundary_shared': None,
            #         'right_boundary_shared': None,
            #         'is_loop': False
            #     })
            ...
            for path_idx in range(len(ref_path)):
                ...
                way_data = self.map_data[way_id - 1]  # Assume way ID starts with 1
