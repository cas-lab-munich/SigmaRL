...
is_global_coordinate_sys = False    # Global coordinate system (bird view) or local coordinate system (ego view)
...
is_observe_boundary_points = False # Whether to observe points on lanelet boundaries or the distance to lanelet boundaries
is_apply_mask = True
...
class ScenarioRoadTraffic(BaseScenario):
    ...
    def reset_world_at(self, env_index: int = None, agent_index: int = None):
        ...
        for env_i in [env_index] if env_index is not None else range(self.world.batch_dim):
            ...
            for i_agent in range(self.n_agents) if not is_reset_single_agent else agent_index.unsqueeze(0):
                ...
                self.corners[env_j, i_agent] = get_rectangle_corners(
                ...
                # Distances from the four corners of the agent to its left and right lanelet boundary
                for c_i in range(4):
                    self.distances.left_boundaries[env_j, i_agent, c_i + 1], _ = get_perpendicular_distances(
                        point=self.corners[env_j, i_agent, c_i, :],
                        polyline=self.ref_paths_agent_related.left_boundary[env_j, i_agent],
                        n_points_long_term=self.ref_paths_agent_related.n_points_left_b[env_j, i_agent],
                    )
                ...
                # Distance from agent to its left/right lanelet boundary is defined as the minimum distance among five distances (four corners, CG)
                ...
                if self.parameters.is_observe_boundary_points:
                    ...
                    # self.ref_paths_agent_related.nearing_points_left_boundary[env_j, i_agent], self.ref_paths_agent_related.nearing_points_right_boundary[env_j, i_agent] = get_nearing_boundaries(
                    #     left_boundary=self.ref_paths_agent_related.left_boundary[env_j, i_agent],
                    #     right_boundary=self.ref_paths_agent_related.right_boundary[env_j, i_agent],
                    #     closest_point_on_ref_path=self.distances.closest_point_on_ref_path[env_j, i_agent],
                    #     n_points_nearing_boundary=self.ref_paths_agent_related.n_points_nearing_boundary, 
                    #     device=self.world.device,
                    #     is_ref_path_loop=self.ref_paths_agent_related.is_loop[env_j, i_agent],
                    #     n_points_long_term=self.ref_paths_agent_related.n_points_long_term[env_j, i_agent],
                    #     sample_interval=1,
                    # )
            ...
            # TODO Add the possibility of computing the mutual distances of agents in env `env_j`
    ...
    def reward(self, agent: Agent):
        ...
        if self.parameters.is_observe_boundary_points:
            # Get nearing points on boundaries
            # self.ref_paths_agent_related.nearing_points_left_boundary[:, agent_index], self.ref_paths_agent_related.nearing_points_right_boundary[:, agent_index] = get_nearing_boundaries(
            #     left_boundary=self.ref_paths_agent_related.left_boundary[:, agent_index],
            #     right_boundary=self.ref_paths_agent_related.right_boundary[:, agent_index],
    ...
    def observation(self, agent: Agent):
        ...
        if self.observations.is_partial:
            ...
            # Corners of nearing agents
            obs_corners_other_agents = self.observations.past_corners[indexing_tuple_1] # [batch_size, n_nearing_agents, n_observed_steps, 4, 2]
            obs_corners_other_agents[mask_nearing_agents_too_far] = self.constants.mask_one # Reference-path mask
            obs_corners_other_agents = obs_corners_other_agents.transpose(1, 2) # [batch_size, n_observed_steps, n_nearing_agents, 4, 2]
            ...
            obs_corners_other_agents = self.observations.past_corners[:, obs_step_start:] # [batch_size, n_observed_steps, n_agents, 4 * 2, 2]
            if self.parameters.is_observe_boundary_points:
        ...
        obs_corners_other_agents_flat = obs_corners_other_agents.reshape(self.world.batch_dim, self.observations.n_observed_steps, self.observations.n_nearing_agents, -1)
        if self.parameters.is_observe_boundary_points:
        ...
        obs_other_agents = torch.cat([
            ...
            # obs_left_lanelet_other_agents_flat if self.parameters.is_observe_boundary_points else None,           # [others] nearing points on left boundaries
            # obs_right_lanelet_other_agents_flat if self.parameters.is_observe_boundary_points else None,           # [others] nearing points on left boundaries
        ...
        obs_list = [
            ...
            self.observations.past_left_boundary[indexing_tuple_3].reshape(self.world.batch_dim, -1) if self.parameters.is_observe_boundary_points else self.observations.past_distance_to_left_boundary[:, obs_step_start:, agent_index].reshape(self.world.batch_dim, -1),      # [own] left boundaries 
            self.observations.past_right_boundary[indexing_tuple_3].reshape(self.world.batch_dim, -1) if self.parameters.is_observe_boundary_points else self.observations.past_distance_to_right_boundary[:, obs_step_start:, agent_index].reshape(self.world.batch_dim, -1),      # [own] right boundaries 
