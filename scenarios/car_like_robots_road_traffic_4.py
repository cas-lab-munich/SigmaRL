...
is_ego_view = True    # Global coordinate system (top-down view) or local coordinate system (ego view)
...
class ScenarioRoadTraffic(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        noise_pri_level = 1 / self.n_agents * 0.2 if not self.parameters.is_ego_view else 0.1
    ...
    def observation(self, agent: Agent):
        ...
        if agent_index == 0: # Avoid repeated computations
            ...
            if self.parameters.is_ego_view: # Ego view
                for a_i in range(self.n_agents):
        ...
        obs_list = [
            self.observations.past_pri[indexing_tuple_3].reshape(self.world.batch_dim, -1) if not self.parameters.is_ego_view else None,                  # [own] priority,
            self.observations.past_pos[indexing_tuple_3].reshape(self.world.batch_dim, -1) if not self.parameters.is_ego_view else None,                  # [own] position,
            self.observations.past_rot[indexing_tuple_3].reshape(self.world.batch_dim, -1) if not self.parameters.is_ego_view else None,                  # [own] rotation,
        ...
        obs = torch.hstack(obs_list if not self.parameters.is_ego_view else obs_list[3:])
