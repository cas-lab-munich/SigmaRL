...
from utilities.helper_scenario import Collisions, Distances, Evaluation, Normalizers, Observations, Penalties, ReferencePaths, Rewards, Thresholds, Obstacles, Timer, exponential_decreasing_fcn, get_distances_between_agents, get_perpendicular_distances, get_rectangle_corners, get_ref_path_for_obstacle_avoidance_scenarios, get_ref_path_for_tracking_scenarios, get_short_term_reference_path, interX, get_point_line_distance, transform_from_global_to_local_coordinate
...
def generate_random_convex_polygon(n: int = 10, x_range: float = 1.0, y_range: float = 1.0, center_point = None, device = torch.device("cpu")):
    """
...
class ScenarioObstacleAvoidance(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        if self.parameters.obstacle_type == "static":
            ...
            center_points, lengths, yaws, _ = get_center_length_yaw_polyline(polyline=random_convex_polygon)

            self.obstacles = Obstacles(
                n=1,
                pos=torch.tensor([[0, 0]] , device=device, dtype=torch.float32),
                corners=random_convex_polygon,
    ...
    def reward(self, agent: Agent):
        ...
        if self.parameters.obstacle_type == "dynamic":
            for obs_i in range(self.obstacles.n):
                # plt.plot(self.corners[0, :, 0], self.corners[0, :, 1])
                # plt.plot(self.obstacles.corners[obs_i, self.timer.step[0], :, 0], self.obstacles.corners[obs_i, self.timer.step[0], :, 1])
    ...
    def observation(self, agent: Agent):
        ...
        if self.observations.is_add_noise:
            ...
            noise_ref_path = torch.randn(self.world.batch_dim, self.ref_paths.n_short_term_points, 2) * self.observations.noise_level #  torch.randn_like generates ramdom noise with the same shape as the input
        ...
        if self.observations.is_global_coordinate_sys:
            ...
            obs_ref_point_norm = ((self.ref_paths.short_term[:,agent_index] + noise_ref_path) / self.normalizers.pos).reshape(self.world.batch_dim, -1)
        else:
        ...
        if self.observations.is_global_coordinate_sys:
            ...
            if "dynamic" in self.parameters.obstacle_type:
                if self.observations.n_nearing_obstacles_observed is not None:
                    # Initialize
                    obs_corners = torch.zeros((self.world.batch_dim, self.observations.n_nearing_obstacles_observed, 4, 2), device=self.world.device, dtype=torch.float32)
                    obs_velocities = torch.zeros((self.world.batch_dim, self.observations.n_nearing_obstacles_observed, 2), device=self.world.device, dtype=torch.float32)
                    ...
                    for nearing_obs_i in range(nearing_obstacles_indices.shape[1]):
                        ...
                        obs_rot_rel = torch.vstack(rot_nearing_obs) - agent.state.rot
                        obs_vel_norm = torch.vstack(vel_nearing_obs).norm(dim=1).unsqueeze(1)
                        obs_velocities[:, nearing_obs_i] = torch.hstack(
                            (
                                obs_vel_norm * torch.cos(obs_rot_rel), 
                                obs_vel_norm * torch.sin(obs_rot_rel)
                            )
                        )
                else:
                    obs_corners = torch.zeros((self.world.batch_dim, self.obstacles.n, 4, 2), device=self.world.device, dtype=torch.float32)
                    obs_velocities = torch.zeros((self.world.batch_dim, self.obstacles.n, 2), device=self.world.device, dtype=torch.float32)
                    ...
                    for obs_i in range(self.obstacles.n):
                        ...
                        # Get the relative velocities of the obstacles
                        obs_rot_rel = self.obstacles.rot[obs_i, self.timer.step].unsqueeze(1) - agent.state.rot
                ...
                obs_corners = transform_from_global_to_local_coordinate(
                    pos_i=agent.state.pos,
                    pos_j=self.obstacles.corners,
                    rot_i=agent.state.rot,
                )
        ...
        obs_list = [
            ...
            obs_ref_point_norm,
            obs_corners.reshape(self.world.batch_dim, -1),
        ]
        
        if self.observations.is_global_coordinate_sys:
            obs_list.insert(
                0,
                (self.observations.past_pos / self.normalizers.pos)[:, index_start:].reshape(self.world.batch_dim, -1)
            )
        
        if self.parameters.obstacle_type == "dynamic":
            obs_list.append(
                obs_velocities.reshape(self.world.batch_dim, -1)
            )
...
if __name__ == "__main__":
    ...
    parameters = Parameters(
        ...
        obstacle_type="dynamic", # [relevant for obstacle-avoidance scenarios] should be one of "static" and "dynamic"
    )
