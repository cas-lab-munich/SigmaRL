...
reward_reach_intermediate_goals = 0 # Reward for reaching intermediate goals
...
penalty_change_steering_direction = 2
...
threshold_change_steering_direction = 10 # [degree]
...
def get_ref_path_for_tracking_scenarios(path_tracking_type, point_interval: float = 0.1, max_speed = agent_max_speed, device = torch.device("cpu"), is_shift_to_origin: bool = True, is_visualize: bool = False):
    if path_tracking_type == "line":
        ...
        path_length = 3.0  # [m] Length along x-axis. Default: 3
...
class ScenarioPathTracking(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        tracking_path, ranges, self.start_pos, self.start_rot, self.start_vel, self.goal_pos, self.goal_rot, is_ref_path_loop, point_extended = get_ref_path_for_tracking_scenarios(
        ...
        # self.viewer_size = (
        #     world_x_dim / (world_x_dim + world_y_dim) * 1000, 
        #     world_y_dim / world_x_dim + world_y_dim * 1000
        # )
    ...
    def observation(self, agent: Agent):
        ...
        if self.observations.is_add_noise:
            ...
            noise_distances_to_ref_path = torch.randn(self.world.batch_dim, 1) * self.observations.noise_level
        else:
            noise_ref_path = 0
        ...
        ##################################################
        obs_ref_path_distances_norm = (self.distances.ref_paths[:,agent_index].unsqueeze(-1) + noise_distances_to_ref_path) / self.normalizers.pos.norm()
